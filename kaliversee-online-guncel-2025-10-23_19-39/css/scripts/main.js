...
(function() {
  ...
  const preloadCriticalResources = () => {
    const criticalResources = [
      { href: './kaliverseestore.jpg', as: 'image', type: 'image/jpeg' },
      { href: './favicon-32x32.png', as: 'image', type: 'image/png' }
    ];
  ...
  // Optimize video loading with lazy loading and performance improvements
  const optimizeVideos = () => {
    ...
    videos.forEach(video => {
      ...
      // Handle autoplay videos with better performance
      ...
      // Add performance optimizations for all videos
      ...
      video.addEventListener('pause', () => {
        if (!document.hidden && !video.ended && video.readyState >= 2) {
          setTimeout(() => {
            video.play().catch(() => {});
          }, 100);
        }
  ...
  const debugLogoLoading = () => {
    ...
    logoImages.forEach((img, index) => {
      ...
      fetch(img.src, { method: 'HEAD' })
        .then(response => {
          if (response.ok) {
            console.log(`Logo ${index + 1} source is accessible`);
          } else {
            console.error(`Logo ${index + 1} source not accessible:`, response.status);
          }
        })
        .catch(err => {
          console.error(`Logo ${index + 1} fetch error:`, err);
        });
    });
  };
  ...
  setTimeout(() => {
    // Load additional resources after 2 seconds
...
// Loader hide after initial paint
...
(function initTheme() {
  ...
  // Start in dark mode by default; respect previously saved preference
...
// Background video controls with lazy loading and performance optimizations
function initVideoLayer() {
  ...
  document.addEventListener('visibilitychange', () => {
    if (document.hidden) {
      ...
      // Resume playing when tab becomes visible with better timing
      setTimeout(() => {
        if (video.paused && !video.ended && video.readyState >= 2) {
          video.play().catch(() => {
            // If resume fails, try again after user interaction
            document.addEventListener('pointerdown', () => {
              video.play().catch(() => {});
            }, { once: true });
          });
  ...
  // Enhanced continuous playback with better error handling
  video.addEventListener('pause', () => {
    if (!document.hidden && !video.ended && video.readyState >= 2) {
      setTimeout(() => {
        video.play().catch((error) => {
  ...
  // Handle video end and restart with better timing
  video.addEventListener('ended', () => {
    video.currentTime = 0;
    setTimeout(() => {
      video.play().catch(() => {
        // If restart fails, try again after user interaction
        document.addEventListener('pointerdown', () => {
          video.play().catch(() => {});
        }, { once: true });
      });
  ...
  // Enhanced periodic check with better performance
  let checkInterval = setInterval(() => {
    if (!document.hidden && video.paused && !video.ended && video.readyState >= 2) {
      video.play().catch(() => {
        // If periodic play fails, reduce check frequency
...
// Simplified Video Animation System (No Controls)
(() => {
  ...
  const videoContainer = mainVideo.closest('.group');
  if (videoContainer) {
    videoContainer.addEventListener('mouseenter', () => {
      enhanceParticles();
      // Add subtle shake effect
      videoContainer.style.animation = 'subtle-shake 0.5s ease-in-out';
    });
...
(() => {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      showNotification('Bir sorun oluştu. Lütfen daha sonra tekrar deneyin veya doğrudan kalidijitalmarketingagency@gmail.com adresine e-posta gönderin.', 'error');
    } finally {
      if (btn) {
